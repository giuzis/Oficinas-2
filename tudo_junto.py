...
# try:
#     while True:
#         print('Distance: ', sensor_distance() * 100)
...
def seekobstacle():
    ...
    while ((sensor_distance()) > 40) and not robotlost:
...
try:
    ...
    while not (iscenteroverblack() and isrightoverblack() and isleftoverblack()) and not robotlost:
        #if (sensor_distance()*100 > 20):
        #     print(sensor_distance())
        #     if (sensor_distance() < 30):# and diminui_velocidade:
        #         print("diminuindo velocidade "+str(sensor_distance()))
        ...
        if False:
            ...
            robot.value = motorright
            time.sleep(0.8)
            while (sensor_distance()) > 30:
                print('vira ' +  str(sensor_distance()))
                robot.value = motorright
                time.sleep(0.5)
                robot.stop()
            ...
            while not iscenteroverblack():
                ...
                if (sensor_distance()) > 40:
                    seekobstacle()
                if sensor_distance() > 15 and sensor_distance() < 20:
                    robot.value = motorforward
                    time.sleep(0.5)
                    robot.stop()
